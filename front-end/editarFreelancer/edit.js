...
dadosfreelancers.addEventListener('submit', (evento) => {
	...
	Promise.all(requests).then(() => {
		...
		freelancersData.set('role', JSON.stringify(selectedRoles));
...
const selectedRolesList = document.getElementById('selectedRoles');

let profileRotation = 0;
let facialRotation = 0;

function updateSelectedRoles() {
	selectedRolesList.innerHTML = ''; // Limpa a lista
...
// Atualiza a lista quando a seleção muda
roleSelect.addEventListener('change', updateSelectedRoles);

// Atualiza a lista na inicialização (caso existam valores pré-selecionados)
updateSelectedRoles();
...
document.addEventListener('DOMContentLoaded', async () => {
	...
	if (freelancerDetails) {
		...
		document.getElementById('birthdate').value = freelancerDetails.birthdate.slice(0, 10) || '';
...
function populateRoleDropdown(selectedRoles) {
