...
function showDetails(freelancer) {
    window.location.href = `./detalhes/?freelancer=${encodeURIComponent(JSON.stringify(freelancer))}`;
}
...
document.getElementById('download').addEventListener('click', () => {
    fetch(`https://api.nkarbits.com.br/freelancers/export?name=${currentName}&city=${currentCity}&role=${currentRole}`)
...
// DownloadImages
	document.getElementById('downloadImagens').addEventListener('click', async () => {
    const checkboxes = document.querySelectorAll('.freelancer-checkbox');
    const buttonText = document.getElementById('downloadImagens').textContent;
    const isSelecting = buttonText === 'Baixar imagem';

    const selectAllButton = document.getElementById('selectAllButton');

    if (isSelecting) {
        checkboxes.forEach(checkbox => checkbox.style.display = 'inline-block');
        document.getElementById('downloadImagens').textContent = 'Baixar selecionados';
        selectAllButton.style.display = 'inline-block';
    } else {
        const selectedFreelancers = [...checkboxes].filter(checkbox => checkbox.checked).map(checkbox => {
        ...
        selectedFreelancers.forEach(async (freelancer) => {
            await downloadFile(freelancer.profilePictureUrl, `${freelancer.freelancerName}_profile.jpeg`);
            await downloadFile(freelancer.facePictureUrl, `${freelancer.freelancerName}_face.jpeg`);
        });

        document.getElementById('downloadImagens').textContent = 'Baixar imagem';
...
document.getElementById('selectAllButton').addEventListener('click', () => {
    ...
    checkboxes.forEach(checkbox => checkbox.checked = !isAnyCheckboxChecked);
});
